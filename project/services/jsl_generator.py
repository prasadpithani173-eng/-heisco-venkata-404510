...
def generate_jsl_log_excel(
    ...
    Matches the layout, summary metrics, and headers shown in the reference document.
    """
    ...
    # Fills
    fill_gold = PatternFill(start_color="FCE4D6", end_color="FCE4D6", fill_type="solid")
    ...
    # Top Title Banner (Row 2, merged B2:M2)
    ws.merge_cells("B2:N2")
    title_cell = ws["B2"]
    title_cell.value = "HSE Action Tracking Register"
    title_cell.font = Font(name=font_family, size=14, bold=True, color="833C0C")
    title_cell.fill = fill_gold
    title_cell.alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[2].height = 28

    # Left Summary Card (Rows 4-7, Cols B & C)
    summary_data = [
    ...
    for i, (label, val) in enumerate(summary_data, start=4):
        ...
        elif "Open" in label and open_actions > 0:
            c_val.fill = fill_status_open
            c_val.font = font_status_open

    # Right Project Info Box (Rows 4-5, Cols L & M)
    ws.cell(row=4, column=11, value="Project Name").font = Font(name=font_family, size=9.5, bold=True)
    ws.cell(row=4, column=11).fill = fill_card_label
    ws.cell(row=4, column=11).border = card_border
    ws.cell(row=4, column=12, value=project_name).font = Font(name=font_family, size=9.5, bold=True)
    ws.cell(row=4, column=12).border = card_border

    ws.cell(row=5, column=11, value="Project Number").font = Font(name=font_family, size=9.5, bold=True)
    ws.cell(row=5, column=11).fill = fill_card_label
    ws.cell(row=5, column=11).border = card_border
    ws.cell(row=5, column=12, value=project_number).font = Font(name=font_family, size=9.5, bold=True)
    ws.cell(row=5, column=12).border = card_border

    # Center Completion Indicator (Row 4-6, Col G & H)
    pct_closed = f"{(closed_actions / total_actions * 100):.0f}%" if total_actions > 0 else "100%"
    ws.merge_cells("G4:H6")
    badge_cell = ws["G4"]
    badge_cell.value = f"● {pct_closed} Closed"
    badge_cell.font = Font(name=font_family, size=13, bold=True, color="006100")
    badge_cell.fill = fill_status_closed
    ...
    for col_idx, header_text in enumerate(headers, start=1):
        ...
        cell.border = dark_border_side_box = Border(
            left=dark_border_side, right=dark_border_side,
            top=dark_border_side, bottom=dark_border_side
        )

    # Data Rows (Row 10+)
    for row_idx, item in enumerate(processed_observations, start=10):
        ...
        row_values = [
            item.get('sn', row_idx - 9),
            item.get('inspection_id', f"HSCO-OB-{row_idx - 9:02d}"),
            "Site Observation",
            obs_date_fmt,
            item.get('area', ''),
            item.get('finding', ''),
