...
def process_and_map_observations(
    ...
    for idx, item in enumerate(sorted_obs, start=1):
        ...
        # Exact Name Mapping Logic:
        # - Observed By = SAFETY OFFICER (Names + Locations Excel)
        # - Status By = ENGINEER (if exists), otherwise SUPERVISOR
        # - Reviewed By = Project Manager (fixed AHMED GHALWASH)
        so_name = item.get('observation_by', '') or matched.get('safety_officer', '')
        
        # Status By: Use Engineer name if available; if not, use Supervisor name
        ...
        for banned in ['Site Supervisor', 'Site Engineer', 'Project Manager', 'Safety Officer', 'nan', 'None']:
            if so_name.strip() == banned:
                so_name = ''
            if status_by_name.strip() == banned:
                status_by_name = ''

        resp_person = item.get('responsible_person', '').strip()
        if resp_person.lower() in ['nan', 'none']:
            resp_person = ''
        ...
        processed_item = {
            ...
            'responsible_person': resp_person or 'Site Supervisor',
...
def split_into_pages(
    ...
    - Observed By = Safety Officer (from Excel based on LOCATION)
    - Status By = Engineer if available else Supervisor
    - Reviewed By = Project Manager Name: AHMED GHALWASH (on every page)
    """
    pages = []
    chunk_size = 3
    ...
    for i in range(0, total, chunk_size):
        ...
        # Extract names from this page's matched observations
        page_safety_officer = chunk[0].get('observed_by', '') if chunk else ''
        page_status_by = chunk[0].get('status_by', '') if chunk else ''
        page_status_by_role = chunk[0].get('status_by_role', 'Supervisor') if chunk else 'Supervisor'
        ...
        page_data = {
            ...
            'status_by_role': page_status_by_role,
